...
file_handler = pkbLogging.make_file_handler("BS13.jsonl", level = 11, format_keys = format_keys)
...
def ordering_b(u, v, L):
    ...
    if len(u) < len(v):
        return (everything, nothing, everything)
    return (nothing, nothing, everything)
orderings = [ordering_a, ordering_a, ordering_b, ordering_b, ordering_a, ordering_a]
...
def prune_prefixes(unresolved, rules, alph):
    ...
    bad_starts = (['T', 'a', 'a', 'a'], ['T', 'A', 'A', 'A'])
    for eqn in unresolved:
        if eqn.left[:4] in bad_starts or eqn.right[:4] in bad_starts:
...
pKB(BS13, max_time = 2000, max_rule_length = 100, prune_prefixes = prune_prefixes, boundary_reduce = boundary_reduce)
